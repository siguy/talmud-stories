...
| 2026-08-31 | **A review UI that records which capability an objection indicts** (Phase B of `work/2026-08-30-review-verdict-axes.md`) | shipped and browser-verified. Axis 1 *is this a story* is the only required question and the only one that decides this capability; extent / confidence / grouping are separate axes defaulting to `right`, so a correct entry stays one click. Every saved verdict carries the label under review and the direction of the error. `build_ruler.py` reads the axes directly, so such a round cannot produce an unclassified note | [`verdict_axes_ui`](../findings/2026-08-31-verdict-axes-review-ui.md) |
