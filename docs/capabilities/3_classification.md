...
| 2026-08-30 | **`build_canonical.py` guarded** — it rebuilds the golden from inputs that no longer include everything the golden holds (a 2026-06-03 round, and five expert-list stories no run ever proposed) | **measured: running it would have deleted 5 stories and *raised* classification recall 0.9085 → 0.9371**, so a score-based safety check points the wrong way. It now refuses any write that is not a strict addition | `21af826`, `6b7fcae`, [Lesson — rebuild scripts](../../lessons/README.md) |
