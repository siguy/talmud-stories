...
|---|---|---|---|
| 2026-09-03 | **Is a proposal really on his list? Measured in the proposal direction for the first time.** The recall aligner returns a search window up to 14 segments wide; a proposal anywhere inside it was silently read as corroborated | **measured: 35 proposals across three tractates sit in a window without overlapping any expert story's own segments — Ketubot 19, Kiddushin 9, Gittin 7 — and 11 are `YES`/`HIGH_CONFIDENCE`.** Recall is unaffected: a generous window is correct for *did we find his stories*, and wrong only when read backwards | [`proposal_credit`](../findings/2026-09-03-loose-window-proposal-credit.md) |
