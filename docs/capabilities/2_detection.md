...
| 2026-09-03 | **Stage 2 checkpointing.** `run_pipeline()` held every page in memory until the tractate finished, so one raise discarded the whole spend — it did twice during the Yevamot run, the second time at page 35 of 106. The detector now hands each completed page to a caller-supplied callback and accepts `resume_stories`; `run_new_tractate.py` persists to `results/stage2/<t>.json` every 10 pages, atomically | **shipped, not a measurement.** A resumed run is byte-identical to an uninterrupted one apart from the `stage2_summary` block that says it resumed — Stage 2 is a pure function of the page, its labels and its two neighbours' context, none of which read back from Stage 2's own output. A page the model **failed** on is never cached, so a resume re-asks rather than inheriting a silence (Lesson 21) | `tests/test_stage2_checkpoint.py`, [`yevamot-first-run`](../findings/2026-09-03-yevamot-first-run.md) |
...
- **Resumability:** Stage 1 and Stage 2 both checkpoint every 10 pages
  (`results/triage/<t>.json`, `results/stage2/<t>.json`). A Stage 2 page is reused only
  when the prompt that produced it is unchanged — the digest is a hash of the built
  prompt, so a prompt edit, a re-fetch, or a changed triage label on the page **or either
  neighbour** invalidates it. Stage 4's own LLM steps (4d stitch, 4f continuation, 4k
  clause spans) are **not** checkpointed and re-run on a resume. A run assembled partly
  from a cache says so in `stage2_summary` and `run_meta.stage2_resumed_pages` — never
  read such a run as a fresh sample of a nondeterministic model.
