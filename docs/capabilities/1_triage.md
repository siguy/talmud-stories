...
**Both figures were located with the 4-gram matcher, retired 2026-09-03.** On the shipped
artifacts — which still carry the *previous* rule — exact anchoring moves Ketubot
**98.0% → 96.6%** and leaves Kiddushin at 95.6%. **The 98.7% / 97.8% live-rule pair above
has not been re-measured under the new matcher**, so read it as the older of the two
numbers here, not the newer.
...
| 2026-09-03 | **Triage recall re-located under exact-phrase anchoring** (no re-run; the same shipped artifacts, read by a matcher that does not reach a neighbouring daf) | **measured: Ketubot 98.0% → 96.6%**, Kiddushin unchanged at 95.6%, Gittin and Yevamot unchanged at 100%. **Two Ketubot stories move from Detection's column to Triage's** — 27a and 51a — whose windows had spilled onto an examined neighbouring daf while their own daf was skipped. Detection-given-triage falls to 90.3%. Nothing about Stage 1's behaviour changed; what changed is which page a story is said to be on | [`cutover`](../findings/2026-09-03-exact-matcher-cutover.md) |
