# RESUME AFTER /clear

**This file is retired as a status document.** It duplicated `STATUS.md` and the two
drifted, which is exactly the problem it was meant to solve.

## Read, in this order

1. **[`STATUS.md`](../STATUS.md)** — where the project is. One page. Start here.
2. **[`tasks/NEXT/`](NEXT/)** — the ready work. Each brief is self-contained: pick one,
   read it, execute it in this session. No other context required.
3. **[`tasks/lessons.md`](lessons.md)** — 24 durable rules. Skim the headings; read any
   lesson a brief cites by number.

That is enough to start. Everything else is reference:

- `validation/feedback/jeff_2026-07-06_feedback_ledger.md` — everything Jeff has said
- `docs/golden/**` — dated findings, immutable once written
- `FOR_SIMON.md` — the plain-English story of how this was built
- `CLAUDE.md` — how to work in this repo

## The rule that keeps this navigable

Status goes in `STATUS.md` and nowhere else. Findings go in a dated doc under
`docs/golden/`. Rules go in `lessons.md`. Ready work goes in `tasks/NEXT/`. When a task
is done, delete its brief and rewrite `STATUS.md` — do not append.
