...
function blank(story) {
  return Object.assign({ is_story: null, display_broken: false, extent_quote: '', note: '' },
                       AXIS_DEFAULTS);
}
...
function setQuote(key, text) { get(STORIES.find(s => s.key === key)).extent_quote = text; }
...
function buildCard(story, idx) {
  ...
  if (isStory === 'yes' || isStory === 'borderline') {
    ...
    more = '<button class="more-link" onclick="document.getElementById(\'more-' + idx
         ...
         + ((v.extent !== 'right')
            ? '<input class="quote-box" placeholder="paste the Hebrew that should be in the story"'
              + ' value="' + esc_attr(v.extent_quote) + '"'
              + ' onchange="setQuote(\'' + story.key + '\', this.value)">'
              + '<div class="quote-hint">Quoting the text is the clearest form &mdash; '
              + 'it says exactly where the story runs, with nothing left to infer.</div>'
            : '')
  ...
  return '<div class="' + cls.join(' ') + '" id="card-' + idx + '">'
    ...
    +     story.key + '\', \'borderline\', ' + idx + ')">Borderline<span class="k">B</span></button>'
