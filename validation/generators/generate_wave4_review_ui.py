...
function buildTextDisplay(story) {{
  // Same order as before the core was extracted: the span notice, the grid,
  // the v9 note, then the cross-page continuation. buildGrid and
  // buildContinuation now live in review_ui_core.py and are shared with the
  // axis review UI, so neither page can drift from the other's display.
  let html = spanNotice(story);
  html += buildGrid(story.page_segments || [], story.start_segment, story.end_segment, story, spanRange);
  html += v9Notice(story);
  html += buildContinuation(story);
  return html;
}}
