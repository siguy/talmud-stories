#!/usr/bin/env python3
"""
Talmud Story Detection v9: v8 baseline + Wave 3 changes from Jeff's 2026-04-23
feedback.

v9 = v8 (Wave 1+2) PLUS:
  - Stage 2 prompt changes (items 1/2/3 — multi-story per page,
    embedded-story few-shots, sharper story-vs-non-story rules)
  - New post-processor edit_text_internal_boundaries (item 4 —
    records text_span_start / text_span_end when the canonical
    introducer or trailing-stam marker falls mid-segment).

Architecture mirrors v7/v8. The class name V7StoryDetector is retained on
purpose so existing call sites only need to swap the import path.
...
{{
  ...
  "reasoning": "brief explanation"
    ...
    def run_pipeline(self, pages: List[Dict],
        ...
        # 4k (Wave 3 Item 4): text-internal boundary editing (text_span_*)
        text_edited = edit_text_internal_boundaries(all_results)
        if text_edited:
            print(f"  Text-span edits: {text_edited} stories got sub-segment spans")
...
def edit_text_internal_boundaries(pages: List[Dict]) -> int:
    ...
    For each real story:
      START: in the FIRST segment, find the EARLIEST canonical introducer
        at a word boundary. If found at offset > 0, record
        story['text_span_start'] = {'segment': i, 'char_offset': N,
        'introducer': str}.

      END: in the LAST segment, find the LATEST canonical trailing-stam
        marker at a full word boundary (after any text_span_start offset).
        If found, record story['text_span_end'] = {'segment': j,
        'char_offset': M, 'marker': str} (slice ends just before the marker).

    The score harness reads only segment indices, so this is score-neutral
    by design. Returns count of stories with at least one text-span field.
    """
    modified = 0
    for page in pages:
        segments = page.get('segments', [])
