---
title: Write the history of each capability
capability: [triage, detection, classification, boundaries, review, publication]
tractate: []
blocked_by: []
awaiting: []
finding:
...
# Write the history of each capability
...
measure, `work/` says what to do — **nothing says what we already tried**, so every
...
| `docs/history/*-PLAN-*.md` | what each wave *intended* — compare against what shipped. |
| `lessons/` | 24 lessons; most were bought by a failure worth recording. |
...
**Done 2026-08-30 (`c98c1a0`).** Six files in `docs/capabilities/`, linked from the STATUS scoreboard. The re-sorting was most of the work: Waves 4/5/5b are Boundaries, Wave 6 is Classification, Waves 1 and 3 split between Detection and Classification. Two failures moved capability with their evidence. Verified mechanically: every cited commit sha resolves, every link resolves. Three defects found while reading and recorded rather than fixed — the triage failed-call default, the missing `NEXT/04` brief (now `work/2026-08-30-review-verdict-axes.md`), and the stale public-site statistics.
