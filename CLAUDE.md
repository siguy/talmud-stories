...
| `scripts/apply_jeff_2026-09-23_verdicts.py` | Jeff's 2026-09-23 verdicts into the Ketubot/Kiddushin goldens — and BORDERLINE for the 10 Ketubot entries where **he** wrote "borderline" and we had rounded to LOW (Lesson 42). Idempotent; a missing entry is an error |
| `scripts/rejudge_twin_additions.py` | **Measures a twin-pass wording change without re-running Stage 2** — re-asks every twin-pass addition on disk under the old (git rev) and new wording. Sees only what the new wording DROPS |
| `docs/findings/2026-09-25-jeff-verdicts-scope-commentary-reports.md` | Jeff's ten verdicts: R-S1 scope, R-B4 commentary, R-C5 bare report, R-C2 settled; why the twin pass re-admitted what Stage 2 rejects; twin pass on by default |
...
| `scripts/build_gittin_golden.py` | **Builds the Gittin golden** — verdicts first, then the blind list, `label_source` on every entry. Reads later rounds (Gittin keys only); writes `OUT_OF_SCOPE` passages to `out_of_scope`, never into `stories[]` |
...
| `scripts/run_parallel_rule_experiment.py` | **The parallel-rule change, run and graded in one command.** Refuses without a `GOOGLE_API_KEY`; `--dry-run` checks the plan with no calls. Bundles the same-code repeat (Lesson 22), the blind-vs-corrections split (Lesson 24) and the direction split, so the parts that make the result trustworthy are not optional |
