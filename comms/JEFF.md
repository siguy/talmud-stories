...
| `jeff:speech-act-policy` | Passages where rabbis only speak are LOW_CONFIDENCE stories today, per his 2026-03-17 rulings; his 2026-07-06 rule reads as *not stories at all*. Should they become NOT_A_STORY, stay LOW_CONFIDENCE, or take a new **borderline** status? | It is a redefinition of a large share of the golden, so it is his call and not ours. Blocks `work/2026-08-30-story-criteria.md` phase 6c **by design**. **Two cases measured 2026-08-31 that point the other way**, and they are his call rather than ours: Kiddushin **44a** `ר' אסי לא על לבי מדרשא` and **58a** `בעא מיניה ר' חייא בר אבין מרב הונא` are on *his own blind list*, we proposed both and then classified both `NOT_A_STORY`, and neither is in our golden — so no review round has ever shown them to him. **58a is now withdrawn from the ask: he has already answered it.** His own 2005 margin comment on that exact passage reads *"Not sure this is a story. Very minimal."* — he agrees with our rejection, twenty years before we made it. **44a stands and is the only one to ask about**, now with two supporting cases from the 2026-09-01 screen — **Kiddushin 25b** (Shmuel finds Rav's students and questions them) and **Kiddushin 27a** (Rabban Gamliel assigns tithe to named individuals): both are specific encounters between named people that our detector proposes and his list omits, so they show the speech-act boundary from the *outside* of the golden rather than the inside ([screen](../docs/findings/2026-09-01-unread-proposals-screened.md)) ([comments harvest](../docs/findings/2026-08-31-kiddushin-comments-harvest.md)). They are why Kiddushin's recall reaching output is 91.1% rather than 93.3% ([finding §4](../docs/findings/2026-08-31-kiddushin-recall.md)). **The sharpest cases arrived 2026-09-01 from Gittin, a tractate nothing of ours has ever touched: 3 of the 4 stories we missed against his blind list are this exact class** — 38b `אמר רבה: בהני תלת מילי` (a dictum), 57a the exchange on the land's fertility (talk, no action), and 57a `אשקא דריספק חריב ביתר` (opens with a custom, Stage 1 labelled it HABITUAL). Our detector rejects all three *because his 2026 rule says to*, and his 2005 list holds all three. A re-run of 57a reproduces both misses exactly, so this is the rule and not noise ([Gittin run](../docs/findings/2026-08-31-gittin-first-run.md)). | **Email 2, item 6** — blocked on our own phase 6a, not on him |
...
| `jeff:axes-round` | Would you review one tractate on the new page, where each entry asks **is it a story** (yes / borderline / no) and, only if you want to say more, whether the **extent**, the **confidence level** or the **grouping** is wrong? | A correct entry is still **one click**. The point is that when something is wrong we learn *which* thing: for months every objection landed in one bucket, so Classification could only be quoted as a range (Ketubot 87.9-94.8%, Kiddushin 67.4-92.1%) and the pooled figure pointed at the classifier when most of the errors were in the boundary code. The page also carries a **Display problem** button, because 3 of your verdicts across two rounds were spent telling us our own renderer was broken. | **Email 1** — it carries `jeff:mishnah-scope` for free: the Mishnah-withheld passages are on the page, filterable. Unblocks [`classification-point-estimate`](../work/2026-08-31-classification-point-estimate.md) |
| `jeff:opening-formula` | When a story is introduced by a citation or attribution formula — `תניא` / `תנו רבנן` / `תא שמע` / `אמר רב יהודה אמר רב` / `כי אתא רב דימי אמר` — is that formula **part of the story** you would publish, or the frame around it? | **The single largest remaining start error, on all three measured tractates.** Where our start differs from his we are almost always one clause late, and the clause we drop is that formula (Gittin 13 of 15 late starts, Kiddushin 6, Ketubot 8). It has to be asked rather than fixed: a rule that extends the start back over the formula **fixes 9 targets and breaks 8** across 470, because he himself starts after the formula about as often as before it — Lesson 15's shape exactly ([analysis](../docs/findings/2026-09-01-gittin-boundary-analysis.md)). | **Email 2** — a boundary question, and Email 1 already carries two |
