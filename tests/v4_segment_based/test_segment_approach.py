...
def build_self_check_prompt(ref: str, stories: List[Dict], segments: List[Dict]) -> str:
    """
    Build a prompt for AI self-validation of identified stories.
    """
    if not stories:
        return ""
...
""")
    ...
    return f"""You previously identified the following as stories from {ref}.
...
def run_self_check(ref: str, stories: List[Dict], segments: List[Dict], model) -> List[Dict]:
    """
    ...
    prompt = build_self_check_prompt(ref, stories, segments)
...
def analyze_page_with_segments(page_data: Dict, model, enable_self_check: bool = True) -> Dict[str, Any]:
    """
